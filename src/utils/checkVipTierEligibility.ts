...
export const checkAndUpdateVipTier = async (
	...
	try {
		const [user, team, profitRate, taskReport] = await Promise.all([
			User.findById(userId),
			Team.findOne({ userId: userId }),
			TireProfitRate.findOne(),
			TaskReport.findOne({ userId: userId }),
		]);
		...
		if (lastVipTier !== highestTier || taskValue !== taskReport?.taskValue) {
			await User.findByIdAndUpdate(userId, { vipTier: highestTier });

			if (taskReport) {
				taskReport.vipTier = highestTier;
				taskReport.taskValue = taskValue;
				if (!taskReport.isActive) {
					taskReport.isActive = true;
				}
